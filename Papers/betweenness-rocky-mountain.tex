...
\title{Real Numbers as Rational Betweenness Relations}
 ...
 \author{James Taylor}
 \address{Arts \& Ideas Sudbury School, 4915 Holder Ave, Baltimore, MD 21214, USA}
 \email{james@aisudbury.org}
 
 \date{July, 1, 2024}

 \keywords{Real numbers, }
 \subjclass{General Mathematics}
...
A Cauchy sequence $x_n$ converges to $x$ if $\xrel$ is defined by $a \xrel b$ exactly when $a:b$ contains the tail of the sequence. In particular, given $a\xrel b$, there must exist an $N$ such that for any $x_n$ with $n \geq N$, there exists $a_n \xrel[x_n] b_n$ such that $a:\{a_n, b_n\}:b$. That such an $N$ can happen follows from the Cauchy criterion. From an interval perspective, the Cauchy sequence could alternatively by given as a double sequence, namely, $x_n, \varepsilon_n$ where the intervals $x_n-\varepsilon_n : x_n + \varepsilon_n$  all overlap and the $\varepsilon_n \to 0$. 

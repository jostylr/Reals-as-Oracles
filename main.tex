...
The\textbf{ $\delta$-spark at $c$}, denoted $c_{\delta}$ is the interval $c-\delta:c+\delta$, where $\delta>0$ is a rational number. A $c_{\delta}$-compatible interval is an interval that is contained in $c_{\delta}$ and strictly contains $c$, i.e., if $e \lt f$ is $c_{\delta}$ compatible then $c-\delta:e\lt c \lt f:c+\delta$. 
...
The Oracle of $r$ is a rule $R$ defined on almost all rational intervals, including singletons, that returns values of 1 and 0, and satisfies the properties listed below.\footnote{Set theoretically, an oracle is a function $R \col \ \mathbb{Q}^2 \to \{0,1\}$ where each pair $(a,b)$ is identified with the inclusive rational interval $a:b$. This viewpoint is suppressed here as the goal is to provide a framework for just-in-time determination. In particular, there may be a rational number $c$ for which the rule has no answer on any interval that has an endpoint of $c$. Another way of viewing the rule is that it is the characteristic function of the set of intervals that contain the real number, a viewpoint which is also not adopted here.} 
\begin{enumerate}
    \item Existence. $R(a:b) = 1$ for some rational interval $a:b$.
    \item Separating. If $R(a\lt b)=1$, then for a given $c \neq a, b$ in $a:b$ and given a halo $\delta > 0$, there exists a $c_{\delta}$ compatible interval $e \lt f$ such that exactly one of the intervals $a:e$, $e:f$, and $f:b$ is a Yes interval and the other two are No intervals. 
    \item Consistency. Assume $c:d$ contains $a:b$. If $R(a:b) = 1$, then $R(c:d) = 1$. If $R(c:d)=0$, then $R(a:b) = 0$. 
    \item Closed. If $c_{\delta}$ is a Yes interval for all rational $\delta>0$, then $R(c:c) = 1$. Additionally, if $R(a:a)=0$, then there exists $\delta > 0$ such that $R(a_\delta) = 0$.
\end{enumerate}
...
The Existence and Separating properties are both essential ingredients in algorithmically refining the knowledge about the real number. The Consistency and Closed properties are more about filling in the blanks and are a bit more atmospheric than algorithmic. In other words, specifying a starting Yes interval and the rule how to do the separating property allows us to develop the real number. The other two properties are there to make both the oracle rule unique and its algorithmic target unique, but are not generally involved in directly producing better approximations. 
...
Because of Consistency, the interval $e:f$ being Yes implies $c_{\delta}$ is a Yes interval. In addition, its complements in $a:b$ are sub-intervals of the complements of $e:f$ so Consistency implies that the No property gets transmitted to the sub-intervals. Thus, we will generally simplify our lives and treat the Separation property for oracles as being about $c_{\delta}$.
    ...
    Without Consistency, we could have a rule which has multiple refinement targets. For example, the rule $R(a \lt b) = 1$ if there exists an integer $n$ such that $n-\frac{1}{4} \leq a < n < b \leq n + \frac{1}{4}$ and No otherwise satisfies existence, separating, and closed. The closed is satisfied since there is nothing which is common to all these intervals. Existence is clear. Separating is pretty easy to establish, noting that there are no Yes intervals covering two integers. It is only consistency which rules this out. 

    The second part of Consistency is asserting that we can fill in the blanks when we know that only one value is consistent with what we know. For example, if $a:b$ contains $c:d$ and $a:b$ is a No interval, then if $R(c:d) = 1$, we would have a contradiction. Consistency says that we therefore must define $R(c:d)=0$. That is, sub-intervals of No intervals are No intervals. When we use this, we will state as Consistency. 
...
For a picturesque language, we define a \textbf{$\delta$-spark} of an oracle to be a rational number $q$ such that $R(q_{\delta})= 1$ for the given positive, rational $\delta>0$. The interval $q_{\delta}$ may be called the $\delta$-halo of $q$. A \textbf{star} of an oracle is a rational number $q$ such that $R(q:q) = 1$ and this would be a stellar oracle. Stars, if they exist, are unique for a given oracle. A \textbf{galactic} oracle is one which has no star of an oracle. The idea is that we are looking at a number from afar and it looks like a point of light in the sky. That point could be a star in our own galaxy (a rational) or it could be a whole new galaxy, and those stars are generally related to one another by rational arithmetic (algebraic extension). As I said, this is just a picturesque way of viewing it and may be good for having interesting language, if one wants that kind of thing. 

We also define the term \textbf{finished oracle} to mean an oracle in which all rational intervals under the oracle rule have a defined answer. For such an oracle, we can almost replace the $\delta$ option in Separation with the option that $R(c:c)=1$. This was the original version of the oracle rule and makes for a simpler presentation when it can be established. 
...
\begin{proof}
 Let $L = b-a$ be the interval's length. Then we take the average of the endpoints: $c = \frac{a+b}{2}$ and use $\delta = \frac{L}{4}$. Since $R$ is Separating, there exists $c_{\delta}$ compatible interval $e \lt f$ such that one of the following holds true:
\begin{enumerate}
...
\begin{proof}
    ...
    Otherwise, we can take $a \lt b$. Choose $n > \max(\log_2 (\frac{L}{\varepsilon}), 1)$ which implies $2^n > \frac{L}{\varepsilon}$ as well as $2^{-n} < \frac{\varepsilon}{L}$.\footnote{We are using a logarithm here for convenience and explicitness. Since we are developing the real numbers, this is a bit out of order. But there should be no doubt that one can keep halving a positive number until it is less than any specified positive number.} Apply the bisection lemma $n$ times, starting with $a \lt b$ and then use the produced Yes subinterval for the next iteration. This leads to at least halving the length each time. Thus, the length of the final interval is at most $2^{-n} * L < \frac{\varepsilon}{L} L = \varepsilon$.
...
\begin{proof}
    Apply the proposition to establish 
\end{proof}
...
We start with establishing two $R$-Yes intervals basically intersect in an $R$-Yes interval.

\begin{proposition}\label{pr:inter}
Let $R$ be an oracle, $a, b, c, d$ be rational numbers such that $a$ is the least and $c \leq d$. If $R(a:b) = 1 = R(c:d)$, then $b \geq c$ and $R(b:c) = 1$.
\end{proposition}

\begin{proof}

By Consistency, $R(a:d) = 1$ since either $b \leq d$ in which case the Yes interval $a:b$ is contained in $a:d$ or $b > d$ in which case the Yes interval $c:d$ is contained in $a:d$. 
...
We now want to establish that $b:c$ is a Yes interval. If $a=c$, then $c:b$ is $a:b$ and is already Yes. If $b=d$, then $c:b$ is $c:d$ and is a Yes interval. We may therefore assume $a<c$ and $b<d$.

We need to separate into two cases. The first case is if $b=c$. Then for any $\delta > 0$ such that $b_\delta$ is strictly contained in $a:d$, the Separating property applies to give use $a:e:b:f:d$. Since we just established that Yes intervals intersect and $a:e$ does not intersect the Yes interval $b:d$, it must be a No interval. Similarly, $f:d$ is a No interval being disjoint from $a:b$. Thus, $e:f$ is a Yes interval. Since this holds for arbitrarily small $\delta$, we have that $b:b$ is a Yes interval by being Closed and thus this is a rooted oracle at $b$. 

Our final case is now $a:c\lt b :d$. Choose $\delta > 0$ such that $b_\delta$ and $c_\delta$ are strictly contained in $a:d$ and disjoint.\footnote{Let $\delta = \min(b-a, d-b, c-a, d-c, c-b)/3$. This should keep the $\delta$ sparks away from the endpoints and $\frac{c-b}{3}$ prevents them from overlapping. This is a footnote as we just need to know that this can be done.} Then Separation tells that there exists intervals $c_\delta$-compatible $e:f$ and $b_\delta$-compatible $g:h$ such that $a:e:f:b$ and $c:g:h:d$ partitions the interval into exactly one Yes interval for each. The following may be helpful to keep in mind which is true by the case we are in and our choices: $a < e < c < f< g < b < h < d$.
...
If both $e:f$ and $g:h$ are No, then $f:b$ is Yes and $c:g$ are Yes. This implies $c:b$ is Yes and, in fact, $f:g$ would be a Yes interval. 

That concludes the analysis of the cases. In all cases, $c:b$ is a Yes interval. 

Thus, we have the intersection is a Yes interval. 
...
\begin{corollary}\label{cor:kissing}
    If $R$ is an oracle, $a < b < c$, and $R(a:b)=R(b:c) = 1$, then $R(b:b)=1$. 
\end{corollary}

This follows from the proof above. 

\begin{corollary}\label{cor:finite-inter}
    For a given oracle, a finite collection of its Yes intervals will have a non-empty intersection which is a Yes interval. 
\end{corollary}

\begin{proof}
We have already established it for pairs. We can thus reduce any finite collection. Indeed, given a collection $\{A_i\}_{i=1}^n$ of Yes intervals, we can define $B_1 = A_1$, $B_i = B_{i-1} \cap A_i$ for $i > 1$. If $B_{i-1}$ is a Yes interval, then the interval $B_i$ exists and is a Yes interval. $B_n$ will then be the intersection of all of the $A_i$ and is a Yes interval.  
\end{proof}
...
\begin{proof}
    Let $L = c-b$. By the bisection approximation, we can find a Yes interval whose length is less than $L$. This interval cannot intersect both $a:b$ and $c:d$ since that would require a length greater than $L$. The interval not intersected by the Yes interval is a No interval. 
\end{proof}
...
\begin{proof}
    Since $a:a$, $b:b$, $c:c$, and $d:d$ are contained in No intervals, they are also No intervals. This implies, by the Closed property, that there is a $\delta$ such that $a_\delta$, $b_\delta$, $c_\delta$, and $d_\delta$ are all No intervals. By the Bisection Approximation, we can find an $R$-Yes interval $e:f$ whose length is shorter than $\delta$. By this choice, if $e:f$ contained $a$, $b$, $c$, or $d$, then it would be wholly contained in the $\delta$-spark for that point. But this is not possible because Consistency would demand that it be a No interval.  Thus, it cannot strictly overlap either $a:c$ or $b:d$. It could be strictly included in one of them, but then it would be contained in a No interval and thus would be a No interval. 
...
We have just used that one method of demonstrating that $R(a:b)=0$ for some given interval $a:b$ is to produce an interval $c:d$ disjoint from $a:b$ such that $R(c:d)=1$. On the other hand, if we could show that all neighborly $R$-Yes intervals intersect $a:b$, but none of them are strictly contained in $a:b$, then either $a$ or $b$ is contained in all such intervals, but not both by the bisection approximation. We could determine which one it is by that approximation. Let's say $a$ is in all of them. Then $R(a:a)=1$ by the Closed property and thus $R(a:b)=1$ as well. This situation is where the usual computational difficulties arise. 

We can extend the buffer idea around a No singleton to a No interval. 

\begin{corollary}
    Let $R$ be an oracle and $a\lte b$ an $R$-No interval. Then there exists a $\delta > 0 $ such that $a-\delta:b+\delta$ is a No interval.
\end{corollary}

\begin{proof}
    Because $a$ and $b$ are in No intervals, we have $a:a$ and $b:b$ to be No singletons. There thus exists a $\delta$ such that $a_\delta$ and $b_\delta$ are No intervals. We then can use the union proposition to say that $a_\delta \cup a:b \cup b_\delta$ is a No interval. That is what was to be shown. 
\end{proof}
...
\begin{proof}
Because $R$ is neighborly, both $a:a$ and $b:b$ are No intervals. Thus, there exists a $\delta>0$ such that $a_\delta$ and $b_\delta$ are No intervals. We can take $\delta$ such that it is also smaller than $\frac{b-a}{3}$.

Then $a:a+\delta:b-\delta:b$ is a partition of the interval $a:b$. We can use the Separation property on this with $\delta/2$. We therefore have rational intervals $e:f$ and $g:h$ that satisfy, by repeated application of the Separation property,  $a:e:a+\delta:f:g:b-\delta:h:b$. This division leads to a single Yes interval of these choices. The intervals $a:e$ and $h:b$ are No intervals as they are contained in the No intervals of $a_\delta$ and $b_\delta$, respectively. Thus, $e:h$ must be a Yes interval and it is strictly contained in $a:b$.

\end{proof}

\begin{proposition}\label{pr:multi}
Given an oracle $R$ and an $R$-Yes interval $a\lt b$ and a $\delta>0$, any finite rational partition of $a:b$ will yield exactly one Yes interval that is strictly between the partition points, possibly including an endpoint, or is an interval containing a partition point whose length is no more than $\delta$.
\end{proposition}
...
If either of the first two is a Yes, then we are done. If not, we iterate by applying Separation to $f_1:b$ using $c_2$ which is in $f_1:b$ due to the choice of $\delta_h$. 

Let us then assume that we have $f_{i-1}:b$ is a Yes interval, where $f_{i-1} < c_i$.  Then Separation using $c_i$ and $\delta_h$ leads to the existence of $e_i:f_i$ containing $c_i$ such that exactly one of $f_{i-1}:e_i$, $e_i:f_i$, or $f_i:b$. If either of the first two are Yes, we are done. If the last is the Yes interval, then iterate again using $f_i:b$ and Separation. 

We do this until we are at $f_n:b$. At that point, we have established that $a:f_n$ is No interval by being a union of overlapping No intervals and $f_n:b$ is Yes and of the form desired. 
\end{proof}


\begin{proposition}\label{pr:no-is-disjoint}
    If an interval $a \lte b$ is a No interval for an oracle $R$, then there exists an $R$-Yes interval disjoint from $a:b$.
\end{proposition}

\begin{proof}
    Since $R(a:b) = 0$, we have $R(a:a) = 0$ and $R(b:b) = 0$. By being closed, there is a $\delta > 0$ such that $a_{\delta}$ and $b_{\delta}$ are No intervals. Thus $I = a-\delta:b+\delta$ is a No interval as the union of overlapping No intervals is a No interval. By the Bisection Approximation, there is a Yes interval $c:d$ whose length is less than $\delta$. If $I$ is disjoint from $c:d$, then so is $a:b$ and we are done. The interval $c:d$ cannot be contained in $I$ as the No interval contains no Yes intervals. If $c:d$ overlaps with $I$ but is not contained in $I$, then that means either $c$ or $d$ is outside of $I$. Let's say $c$ is. Then $c_\delta$ both contains $c:d$ and is disjoint from $a:b$ since the distance from $c:c$ to $a:b$ is greater than $\delta$. Thus, $c:d$ is both disjoint from $a:b$ and a Yes interval, as desired. 
\end{proof}
...
\begin{enumerate}
    ...
    \item Separating. Assume $R(a:b)=1$ and let $c$ be strictly contained in $a:b$ and the halo $\delta >0$ be given. 

    If $c:c$ is a Yes interval, then $c:c$ is the intersection of a community of elements in the fonsi. Since the intersection must intersect each element of the fonsi, we have $c$ is in all of the elements. Thus, any interval that does not contain $c$ is No. We can therefore establish that $c_\delta$ is Yes and the intervals outside of it are No. 

    We can therefore assume there exists an interval $B$ in $\mathcal{I}$ which does not contain $c$. Let $L$ be the distance from $c$ to $B$. 
    
    Let $A$ represent an intersection of elements of $\mathcal{I}$ which is contained in $a:b$; $A$ exists by definition of $a:b$ being a Yes-interval.   Consider $A \cap B$. By Proposition \ref{pr:fonsi-inf-inter}, this intersection is non-empty. $A \cap B$ will be contained in $a:b$ since $A$ is.  

    Let $\varepsilon = \min(L/2, \delta)$. Then $c_\varepsilon=e:f$ is contained in $c_\delta$ and disjoint from $A \cap B$. If $e:f$ were to contain an intersection, then each of the elements would have to intersect $B$. But that would mean they would all contain at least the closest endpoint of $B$. Then the intersection would include that endpoint and not be contained in $e:f$ which is disjoint from $B$. Let's assume $A \cap B$ is contained in $a:e$. Then $a:e$ is a Yes interval, but $e:f$ is No as is $f:b$ by the same logic of any intersection in $f:b$ not being able to intersect $B$. 
    ...
    First, if there exists an element in the fonsi that does not contain $c$, say $a:b$ where $b$ is the closest number to $c$. Consider $\delta = |b-c|/2$. By assumption, $c_\delta$ contains the intersection of a community of intervals. But we have shown that that intersection must intersect every element of the fonsi. But $a:b$ is disjoint from $c_\delta$ and thus cannot intersect the intersection. Thus, every element must contain $c$. 

    Next, consider $p \neq c$. Then consider $\delta = |c-p|$. Then $c_\delta$ does not contain $p$ and therefore $p$ is not in the intersection that is supposed to be contained in $c_\delta$. So the intersection of all of the fonsi elements is $c$ in this scenario. 

    Finally, we need to establish that if $a:a$ is No, then there exists a $\delta >0$ such that $a_\delta$ is No. Since $a:a$ is No, there must exist an element of the fonsi which does not contain $a$. Let $c:d$ be such an element with $d$ the closest element of $a$. Then $\delta = |d-a|/2$ should work. Indeed, since the intersection of a community of intervals must intersect the elements of the fonsi, any intersection contained in $a_\delta$ would have to intersect $c:d$ but that is not possible. So no such intersection is in $a_\delta$ and it is a No interval. 
...
\begin{enumerate}
    ...
    We will side with the conventions of interval analysis and use the exponent to represent it being applied to each member of the interval. That is $a:b * a:b \neq (a:b)^2$ in general though the former always contains the latter. Being skeptical of algebraic formulations in interval analysis is a necessity while it becomes less important in the theoretical refinement of oracle arithmetic. 

...
\begin{abstract}
Irrational numbers cannot be known in a precise way in the same fashion that rational numbers can be. But one can be precise about the uncertainty. The assertion is that if one can ascertain whether or not a real number is in any given rational interval, then one can claim to know the real number. This insight leads to a two tiered definition of real number. The top tier is the idealized rational betweenness relation which details what properties a relation on rational numbers must satisfy to qualify as a real number. The other tier is the practical approach to real numbers. These are non-unique procedures that give definite pathways to bringing forth the rational betweenness relations. Through these procedures, it will be established that the rational betweenness relations satisfy the axioms of the real numbers. 
\end{abstract}


Real numbers are typically defined as completed infinite objects, yet in practice they are accessed only through finite computations. This creates a tension in which formal definitions present real numbers as fully determined entities while computational methods only ever produce partial, interval-based information. Decimals, Cauchy sequences, and related constructions all reflect this reality, but they obscure the interval information and often introduce significant redundancy or ambiguity in representation. Dedekind cuts lean more into the completed infinite object presentation with a corresponding decrease in practical usefulness.

The viewpoint adopted in this paper is that this interval-based knowledge is the correct foundation and not just an approximation. To know a real number is to know whether it lies within any given rational interval. This leads naturally to a two-tiered framework. On the one hand, there is an idealized notion of a real number as a rational betweenness relation which is a structure that records exactly which rational intervals contain the number. On the other hand, there are practical representations, called oracles, which are procedures that answer interval membership queries through finite means.

A central feature of this framework is that while the idealized representation is unique, the practical representations are not. Different procedures may yield different pathways for refining information about the same real number. This non-uniqueness is a reflection of the computational reality of working with real numbers.

Section 1 introduces notation for rational intervals and betweenness which provides a convenient language for the rest of the paper. Section 2 defines rational betweenness relations and presents them as the canonical representation of real numbers. Section 3 introduces oracles, which serve as the computational counterpart to these relations, and states the properties they must satisfy. Oracles themselves are split into the fully realizable proto-oracles and their own idealized counterparts which are the full oracles. 

Section 4 develops examples of oracles and shows how familiar constructions of real numbers fit into this framework. In particular, two broad classes emerge. Testing oracles determine interval membership directly, while algorithmic oracles operate by generating families of increasingly refined intervals. The latter are formalized through refinement funnels, which capture the common structure underlying many numerical procedures.

Section 5 establishes key properties of oracles, including computational techniques such as the bisection algorithm. Section 6 connects oracles to rational betweenness relations and defines when two oracles represent the same real number. Section 7 completes the development by defining arithmetic and proving that the resulting structure satisfies the axioms of a complete ordered field.

Throughout the paper, attention is given to distinguishing what can be known through finite, computational means versus that which is more in the realm of the proof and infinity. The goal is not to restrict to computable procedures, but to clarify what information could be obtained through finite interaction with an oracle. Proto-oracles are generally what can be known by computational means and oracles are where proof becomes important. 



---

Real numbers are a fundamental part of mathematics. There are many different definitions and they each have their own inadequacies. This paper will present a new definition based on intervals. Intervals have been used before, but the definitions here are more flexible and revealing in dealing with the tension of a real number as an infinite entity being worked with by finite means. 

Section 1 introduces some notation. In particular, the notation of $ a \rint b \rint c$ is that $b$ is between $a$ and $c$ without having to be explicit about whether this is $a \leq b \leq c$ or $c \leq b \leq a$. Being able to speak about betweenness in a convenient manner smooths out much of the presentation in this paper. Also of note is $a \rint b$ to speak about the inclusive interval of rational numbers between $a$ and $b$. 

Section 2 defines a rational betweenness relation. This paper takes this to be what a real number actually is. It is idealized in that it is an infinite set of pairs of rational numbers that cannot be generally presented in a finite manner. What it does have to offer, similar to Dedekind cuts, is a canonical representation of the real number. It improves on Dedekind cuts as it focuses on the actual desired information about a real number. 

Working practically with real numbers requires a different structure. Section 3 introduces oracles which are procedures that give Yes or No answers to the question of whether the real number it represents is contained in a given rational interval. There are various properties such a procedure must satisfy to qualify as a representative of a real number. 

For both rational betweenness relations and oracles, most of the properties are bookkeeping properties. The core property for both of them is the ability to subdivide a Yes interval into a Yes interval and a No interval. This is the Separation property and this is what makes the machinery focus in on a single real number. Rational betweenness relations are precise objects which is why they are not suitable for practical uses. Oracles allow fuzziness which does make them useful in practical settings. 

Section 4 gives examples of oracles and demonstrates how more familiar presentations of real numbers fit. It starts off with rational numbers and explores different various oracles that could be used for a given rational number. This can be useful to see how the mechanisms actually work. In contrast, there is only a single rational betweenness relation representing a rational number, namely, it is the set of endpoints of intervals that contain the rational number. After exploring rational numbers, two main types of oracles are explored. 

The testing oracles are those oracles that can directly test whether, given a rational interval, the real number ought to be in it. For example, given a non-negative rational interval $a\rint b$, it ought to contain $\sqrt{2}$ exactly when $a^2 \rint 2 \rint b^2$. These oracles fit naturally with finding the least upper bound of a set and are closely linked with what Dedekind cuts work well with representing. Filter representations of real numbers, such as that of minimal Cauchy filters \cite{ittay-2015}, also fit well with the logic of testing oracles. 

The algorithmic oracles are those that do their work using a procedure that produces ever smaller intervals all of which overlap. Such procedures, referred to as refinement funnels, are common in numerical approximations of real numbers. Cauchy sequences are the main example to have in mind in which the intervals containing the tail of the sequence is the object of actual interest. Decimal presentations of real numbers can also be thought of as a basis of algorithmic oracles where the intervals are the intervals of uncertainty around the decimals. Other interval definitions such as Bachmann's nested intervals or the constructivists fine families \cite{bridger} also fit into the algorithmic oracles. 

Section 5 is a technical section that establishes various desired properties that can be deduced from the oracle definition. The Bisection Algorithm is the core computational technique. For algorithmic oracles, this is less important as they are already generally based on being able to make smaller and smaller intervals, but the Bisection Algorithm makes a standardized path that is important for the testing oracles and for being able to make some computational claims about what can and cannot be guaranteed. The fact that all of the intervals of interest are intersecting has a variety of useful forms that are detailed in that section as well. 

Section 6 makes the explicit connection between rational betweenness relations and oracles. For a given real number, there is exactly one rational betweenness relation, but there are infinitely many oracles that could represent it. An equivalence relation on oracles is established using their associated rational betweenness relation as the anchor. Conditions for when the equivalence applies are explored. 

Section 7 is where these objects are established as the real numbers. The arithmetic of rational betweenness relations is the arithmetic of intervals with some limiting behavior added. The completion step is what the oracles help facilitate, particularly in the form of refinement funnels. All of the necessary field properties are established as is the set of rational betweenness relations being complete. 
...
A third approach is that of Dedekind cuts. One version has a real number be the set of all rational numbers strictly less than itself. This works well formally and defines a unique representative. But it is a little unclear what is being computed. In particular, to get a sense of what the number actually is, there has to be some computation of an upper bound on that set. Pairing an element of this set with an upper bound then provides an interval which ought to contain the real number. Pairing all the lower bounds with each of the upper bounds leads to the idea of the set of all intervals that contain the real number. This is what this paper calls rational betweenness relations and is what is asserted to be the singular entity of what a real number is. Its equivalence to Dedekind cuts is detailed in \cite{taylor24dedekind}. 

A more recent approach is that of minimal Cauchy filters \cite{ittay-2015}. A filter is a structure that given any set in it, everything that contain the given set is also in the filter; it is closed above. The idea of a minimal Cauchy filter is that it contains a family of intersecting intervals that get arbitrarily small (the Cauchy part) and it is minimal in the sense that any filter containing those base sets will contain the minimal one. For rationals, the minimal Cauchy filter does not include the rational singleton. This is distinct from ultra-filters which is a slightly different approach to trying to create a new space of objects using filters. This is in the world of sets so that given any particular element of the filter,  it can be quite messy, potentially infinite in width, and far away from any sense of being close to the number of interest with no intrinsic way of codifying that. 
...
A common situation for real number estimates is that the real number is computed as a sequence of intervals. For example, the $n$-th root of the positive rational $q$ can be computed using Newton's method. Let $a_0 >0$ be some positive rational. Given $a_m$, define $b_m = q/a_m^{n-1}$. Because $a \rint 1 \rint 1/a^{n-1}$ applies for any positive rational $a$, it is the case that  $x^n/q \rint 1 \rint (q/x^n)^{n-1}$ holds for positive $x$ and $q$. This implies $x^n \rint q \rint q^n/x^{n^2 -n}$ which, letting $x = a_m$, translates into  $a_m^n \rint  q \rint  b_m^n$. The iteration is defined as $a_{m+1} = a_m + (b_m - a_m)/n$. This is what results from applying Newton's method to $x^n - q$. The oracle procedure $R$ would then be to compute $m, a_m, b_m$ such that $|a_m \rint  b_m| < \delta$ and to define $R(a\rint b, \delta) = (k, a_m\rint b_m)$ where $k=1$ if $a_m\rint b_m$ intersects $a\rint b$ with $k =0$ otherwise. Due to the length, $a_m\rint b_m$ will be in $\halo{a\rint b}$ if it intersects $a\rint b$. 
...
More general uses of Newton's method can be discussed, but it is more efficient to discuss a general construct. A \textbf{refinement funnel} will be a procedure $F$ that takes in a positive rational $\varepsilon$ and returns an interval whose length is less than $\varepsilon$. A further requirement is that all output intervals of $F$ must intersect. It may be the case that $F$ returns various different intervals for the same $\varepsilon$; the ideas of core input and core output can be implemented here as well. 

\begin{proposition}\label{os-linked}
    Given a refinement funnel $F$, there is an oracle whose prophecies are exactly the returned intervals of $F$. 
\end{proposition}
...
\subsection{Fonsis and Refinement Funnels}\label{os-fonsis}
...
These are closely related to the refinement funnels of Section \ref{os-sect-algo}. The range of a refinement funnel is a fonsi by definition of both of these terms. Given a fonsi, one can abstractly define a refinement funnel as one that, given a positive rational number, the return value is one of the intervals in the fonsi whose length is less than that given number.
...
\begin{proof}
    ...
    Because the elements of the fonsi all pairwise intersect one another, it is a refinement funnel. Thus, Proposition \ref{os-linked} applies and yields the desired oracle. 
\end{proof}
...
\begin{corollary}
    Given an oracle, the complete set of all Yes intervals forms a maximal fonsi which in turn forms a maximal refinement funnel.
\end{corollary}

\begin{proof}
A maximal fonsi is one which is not contained in any other fonsi. That is equivalent to the fonsi having the property that every interval not in the fonsi is disjoint from at least one element of the fonsi. Since any interval that intersects arbitrarily small prophecies is a Yes interval, for an interval to not be a Yes interval, then non-constructively, there must exist a prophecy that the interval is disjoint from. Thus, that interval cannot be added to the fonsi. 

A maximal refinement funnel is one such that no interval can be added to the range and maintain the intersection compatibility. The refinement funnel associated with the maximal fonsi is a maximal refinement funnel. 
...
For oracles, the canonical refinement funnel is a little abstract and somewhat relies on being able to make a random choice across an infinite set of intervals that may not be explicitly known. An alternative is to use the oracle and the Bisection Algorithm logic to give a more concrete refinement funnel. That is, given a rational length, bisect a given starting interval, say the one associated with Existence, and repeatedly use Separation to form an interval smaller than the given length. To the extent that the oracle procedure is concrete and reproducible, so will this procedure be particularly if given a concrete choice of which of the subintervals to examine first. 

\begin{proposition}
    Given an oracle $R$ and a refinement funnel $F$, the generated oracle from $F$ will be equivalent to $R$ if and only if the range of $F$ is a set of $R$-Yes intervals.
\end{proposition}

A refinement funnel $F$ is said to be $R$-compatible when its generated oracle is equivalent to $R$. 

\begin{proof}
    The generated oracle, $R_F$, has the property that the prophecies are $R$ is the range of $F$. For two oracles to be equivalent, all of their prophecies must intersect and are Yes intervals of the other oracle. So if $R_F$ is equivalent to $R$, then the range of $F$ is a set of $R$-Yes intervals. If the range of $F$ is a set of $R$-Yes intervals, then they intersect every prophecy of $R$. Since these are the prophecies of $R_F$, it is the case that $R$ and $R_F$ are equivalent as oracles. 
\end{proof}



\begin{corollary}
    Given equivalent oracles $R$ and $R'$, a refinement funnel $F$ is compatible with $R$ if and only if it is compatible with $R'$.
\end{corollary}

\begin{proof}
    Equivalent oracles have the same Yes intervals so if $F$ is compatible with one of the oracles, then it is for the other since its range will be a set of Yes intervals for both. 
\end{proof}

Two refinement funnels are said to be equivalent if they generate equivalent oracles. This can be denoted by $F_x \equiv F_y$. They are compatible if each of the output intervals of one funnel intersects each of the output intervals of the other funnel. 

\begin{proposition}\label{os-prop-refine-equiv}
    Two refinement funnels are equivalent if and only if they are compatible. 
\end{proposition}

\begin{proof}
    Equivalent oracles have the same set of Yes intervals. The ranges of refinement funnels are subsets of the Yes intervals of the generated oracles. Thus, if the generated oracles are equivalent, then the ranges of the refinement funnels are subsets of the same set of Yes intervals and thus pairwise intersect. In the other direction, if they do pairwise intersect, then the prophecies of the generated oracles are the ranges. If two oracles have prophecies that pairwise intersect, then they are equivalent oracles. 
\end{proof}
...
The proof that the above is an oracle is straightforward, but longer than desired and not different than what has been done earlier. Instead, the path chosen below is to use refinement funnels which work well with requiring intervals of a restricted size. The oracle and rational betweenness relation then follows from the refinement funnel. 
...
\begin{proposition}
    Given oracles $R_x$, $R_y$ and any compatible refinement funnels $F_{x}$ and $F_{y}$, respectively, the procedure $F_{x+y} (\delta) = F_x (\delta') \oplus F_y (\delta'')$ is a refinement funnel where $\delta' + \delta'' \leq \delta$. 
\end{proposition}

Section \ref{os-sect-equiv} ends with a discussion of compatible refinement funnels. 

\begin{proof}

    Let $\delta > 0$ be given and compute $F_{x+y}(\delta)$. 

    By definition, this is $F_x(\delta') \oplus F_y(\delta'')$ where $\delta'$ and $\delta''$ satisfy the requirements of the statement. 

    $F_x( \delta')$ will produce an $x$-interval whose length is less than $\delta'$; similarly for $F_y(\delta'')$. As discussed before, the length of an interval of a sum of intervals is the sum of the lengths. This gives $|F_x(\delta') \oplus F_y(\delta'')| < \delta'+\delta'' \leq \delta$. Thus, $F_{x+y}$ produces arbitrarily small intervals. 
         
    By the lemma, the range of $F_{x+y}$ will pairwise intersect. Thus, $F_{x+y}$ is a refinement funnel. 
    
\end{proof}


The associated oracle with this refinement funnel will be denoted as $R_{x + y}$; it is equivalent to the oracle sketched previously as can be seen by comparing prophecies. The associated rational betweenness relation will simply be written as $x+y$. 

Given two oracles $R_x$ and $R_y$, let an absolute bound $M$ mean a number such that there exists an $x$-interval $a:b$ and $y$-interval $c:d$ such that $\max(|a|, |b|, |c|, |d|)  < M$. Furthermore, let $\varepsilon_{x,M}$ represent a length such that any $x$-interval whose length is less than $\varepsilon_{x, M}$ has the property that the interval is absolutely bounded by $M$.  

\begin{proposition}
    Given oracles $R_x$, $R_y$, an absolute bound $M$, and any compatible refinement funnels $F_{x}$ and $F_{y}$, respectively, the procedure $F_{xy} (\delta) = F_x(\delta') + F_y(\delta'')$ is a refinement funnel where $M(\delta' + \delta'') \leq \delta$, $\delta' < \varepsilon_{x, M}$, and $\delta'' < \varepsilon_{y,M}$. 
\end{proposition}

\begin{proof}

    Let $\delta > 0$ be given and compute $F_{xy}(\delta)$.

    By definition, this is $F_x(\delta') \otimes F_y(\delta'')$ where $\delta'$ and $\delta''$ satisfy the requirements of the statement. Let $F_x(\delta') = a:b$ and $F_y(\delta'') = c:d$. As $\delta' < \varepsilon_{x, M}$ and $\delta'' < \varepsilon_{y,M}$ hold true, it is the case that $a:b$ and $c:d$ are absolutely bounded by $M$. Their lengths are also bounded by $\delta'$ and $\delta''$ respectively. As discussed above, this implies that $|a:b \otimes c:d| \leq M(|a:b| + |c:d|) < M(\delta' + \delta'') \leq \delta$. Thus, $F_{xy}$ produces arbitrarily small intervals. 
         
    By the lemma, the intervals in the range of $F_{xy}$ will pairwise intersect. 
    
    $F_{xy}$ is, therefore, a refinement funnel. 

    
\end{proof}

The associated oracle to the refinement funnel will be denoted as $R_{xy}$; it is equivalent to the oracle sketched previously as can be seen by comparing prophecies. The associated rational betweenness relation will simply be written as $xy$. In the case that the lack of a symbol is awkward, the notation $x * y$ may be used as well. 

One also has to establish that using different compatible refinement funnels as operands results in equivalent oracles.

\begin{proposition}
    Let $F_x \equiv F_x'$ and $F_y \equiv F_y'$ be pairs of equivalent refinement funnels. Then $F_{x \cdot y} \equiv F_{x \cdot y}$. 
\end{proposition}

\begin{proof}
The task is to show that the intervals of $F_{x \cdot y}$ intersect the intervals of $F'_{x \cdot y}$. By the equivalence, all the intervals of $F_x$ intersect all the intervals of $F'_x$ and all the intervals of $F_y$ intersect the prophecies of $F'_y$. Let $a\rint b \in F_{x \cdot y}$ which means there exists $c\rint d \in F_x$ and $e\rint f \in F_y$ such that $a\rint b = c\rint d \odot e\rint f$. Similarly, let $a'\rint b'\in F'_{x \cdot y}$ with $c'\rint d' \in F'_x$, $e'\rint f' \in F'_y$ such that $a'\rint b' = c'\rint d' \odot e'\rint f'$. Then by assumption of the equivalences, there exists $p$ that is in both $c\rint d$ and $c'\rint d'$ along with a $q$ in both $e\rint f$ and $e'\rint f'$. Then $p \cdot q$ will be in both $a\rint b$ and $a'\rint b'$. Thus, they intersect. This shows that all intervals of $F_{x \cdot y}$ intersect all the intervals of $F_{x' \cdot y'}$ which implies they are equivalent. 
\end{proof}

\begin{corollary}
    Given equivalent oracles $R_x \equiv R'_x$ and $R_y \equiv R'_y$, the oracles $R_{x \cdot y} \equiv R'_{x \cdot y}$.
\end{corollary}

\begin{proof}
    Given the equivalent oracles, the refinement funnels compatible with them are equivalent. Thus, the proposition applies to have the generated refinement funnels being equivalent. By Proposition \ref{os-prop-refine-equiv}, this implies their generated oracles are equivalent. 
\end{proof}

The equivalence implies that they represent the same completed rational betweenness relation.

Having established that these are oracles, it is necessary to check the field properties. The idea is to demonstrate that the oracles as expressed in different ways by the properties are, in fact, equivalent and thus the properties hold. One can use intersecting prophecies of the oracles, but it is both sufficient and equivalent to argue that the respective refinement funnels are compatible. Often, they are identical. 

\begin{enumerate}
    \item Commutativity. To show $x\cdot y = y \cdot x$, consider $F_x \cdot F_y$ versus $F_y \cdot F_x$. If $F_x(\delta') = a \rint b$ and $F_y(\delta') = c \rint d$, and letting $a \rint p \rint b$ and $c \rint q \rint d$, it is clear that since $p \dot q = q \dot p$, that $a:b \cdot c:d = c:d \cdot a:b$. Thus, $F_x \cdot F_y = F_y \cdot F_x$. Thus, $F_{x \cdot y} \equiv F_{y \cdot x}$, establishing that commutativity holds. 
    \item Associativity. To show $(x\cdot y) \cdot z = x \cdot (y \cdot z)$, the interval fact to use is that $(a\rint b \odot c\rint d) \odot e\rint f = a\rint b \odot (c\rint d \odot e\rint f)$. Using the same logic as before, it is the case that $F_{(x \cdot y) \cdot z} = F_{x \cdot (y \cdot z)}$. 
    \item Identities. Let $R_0$ be the Singular Oracle of $0$ and $R_1$ be the Singular Oracle of $1$. The only refinement funnel that generates the singular oracles is $F_0(\delta) = 0:0$ and $F_1(\delta) = 1:1$. The task is to show $x+0=x$ and $1x  = x$.  Since $F_x(\delta') \oplus F_0(\delta'') = a\rint b \oplus 0\rint 0 = a\rint b$, the refinement funnels $F_{x+0}$ and $F_x$ are compatible and generate equivalent oracles. As $F_x(\delta') \otimes F_1 (\delta'') = a\rint b \otimes 1\rint 1 = a\rint b$, the refinement funnels $F_{x + 1}$ and $F_x$ are compatible and generate equivalent oracles.  This establishes the additive identity as 0 and the multiplicative identity as 1. 
    \item Distributive Property. As previously mentioned, $( a\rint b \otimes ( c\rint d \oplus e\rint f) \subset (a\rint b \otimes c\rint d) \oplus (a\rint b \otimes e\rint f)$. Thus, $F_{x(y+z)} \subset F_{xy + yz}$ implying they are compatible refinement funnels that generate equivalent oracles which demonstrates that the Distributive Property holds. 
    \item Additive Inverse. Given oracle $x$ with compatible refinement funnel $F_x$, define $-x$ to be the oracle generated from the refinement funnel $F_{-x} (\delta ) = \ominus F_x(\delta)$. The procedure $F_{-x}$ is a refinement funnel as $|a \rint b| = |\ominus a \rint b|$ and if $p \in a \rint b$, then $-p \in a \rint b$. Thus, there are arbitrarily small outputs and all the outputs intersect, using those facts being true of $F_x$.

    Therefore $F_{-x}$ as defined above is a refinement funnel which generates an oracle $R_{-x}$ which generates the betweenness relation denoted as $-x$. 
    
    To verify that this is the additive inverse, note that $F_x(\delta') \oplus F_{-x}( \delta'') = F_x(\delta') \oplus  (\ominus F_{x}( \delta'') ) = a \rint b \oplus (\ominus c\rint d ) = a \rint b \oplus -c \rint -d$. Since $F_x$ is a refinement funnel there exists $p$ such that $a : p : b$ and $c : p :d$ implying $-c: -p : -d$. Thus $p -p = 0$ is an element of  $F_x(\delta') \oplus F_{-x}( \delta'')$. This is true for all $\delta'$ and $\delta''$. Hence, $0$ is an element of all of the outputs of $F_{x -x}$. This means they intersect $0:0$ which implies this is equivalent to the singular oracle $0$'s refinement funnel. Thus, $x-x = 0$. 
    
    \item Multiplicative Inverse. For the multiplicative inverse of $x$, it is slightly more complicated. The multiplicative inverse only exists if $x \neq 0$. Being not $0$, there exists at least one prophecy $u\rint v$ of $x$ that does not contain 0. Define $F_{1/x}(\delta) = 1 \oslash (u \rint v \cap F_x(\delta')$). The intersection will be a Yes interval of $x$ that excludes 0. 
    
    Let $m = \min(|u|, |v|)$ and $a \rint b = u \rint v \cap F_x(\delta)$. Then $|a|, |b| > m$ and $ab > 0$. Thus, $|1 \oslash (a \rint b)| = |1/a : 1/b| = |b-a|/ab < |b-a|/m^2 < \delta'/m^2$. Thus, if $\delta' < \min(m^2 \delta, \delta)$, then $|1 \oslash (a \rint b)| < \delta$. This shows that $F_{1/x}$ yields arbitrarily small intervals. 

    As for them intersecting, assume $a:b$ and $c:d$ are two outputs of $F_{1/x}$. This implies that $1/a:1/b$ and $1/c:1/d$ are Yes intervals of $x$. Thus, they intersect. Let $p$ be in that intersection. Then $1/a : p : 1/b$ implies $a : 1/p : b $; the betweenness relation holds as $a$ and $b$ have the same sign. Similarly, $c: 1/p : d$. Thus, $1/p$ is common to both intervals and they do intersect. 
        
    Therefore $F_{1/x}$ as defined above is a refinement funnel which generates an oracle $R_{1/x}$ which generates the betweenness relation denoted as $1/x$. 

    To verify that this is the multiplicative inverse, compute $F_{x*1/x}$. As with the additive inverse, compute $F_x(\delta') \otimes F_{1/x}(\delta'') = a\rint b \otimes c\rint d$. There exists $p$ that is common to both $a\rint b$ and $1 \oslash (c\rint d)$. Thus, the multiplication of these intervals leads to $p *1/p = 1$ being in the product interval and therefore $F_{x*1/x}$ is compatible with the refinement funnel of the Singular Oracle of 1. Thus, $1/x$ is the multiplicative inverse of $x$; this exists for all $x \neq 0$. 
...
The idea was quite simple:  a real number is best represented by the intervals that contain it. To unpack this idea, several levels of tools were used.

The idealized version of real numbers are the rational betweenness relations. While Dedekind cuts emphasize, in some sense, being away from the real number, the relations suggest enclosing the real number. The idea is that if one knows which rational intervals contain the real number, then it is reasonable to say that one knows the real number. Unfortunately, this is not achievable for all real numbers by finite processes. 

A more practical version is that of the oracles. They are the rational betweenness relations with a bit of fuzziness built in as well as having more of a just-in-time emphasis. One does computations upon demand and there is no suggestion of being presented an infinitely complete object. 

There are infinitely many oracles that can represent the same rational betweenness relation. This defines a useful equivalence relation on oracles, a relation that can be established by considering the intersections of the prophecies. 

There is a third layer which is the layer of objects that generate oracles. There are two major divisions in how real numbers arise. 

The first is that of the algorithmic way. Cauchy sequences are based on this idea as are most of the definitions of real numbers. The presentation here used refinement funnels and fonsis to explore the idea of objects that are notionally shrinking and overlapping. The fonsis can be useful abstractly as the range of the refinement funnels, but it is the refinement funnels that enabled defining arithmetic of real numbers efficiently. 

The other way oracles get generated is by testing intervals. This often arises in connection with inequalities. Dedekind cuts are inspired by this viewpoint. A common way to define testing oracles is to use a function $f$ which, on a defining interval of interest, is negative on one side of the desired real and positive on the other side. This is naturally expressed by oracles. 

It is in this way that oracles bridge the main disparate identities of real numbers. They bring a practical, finite approach to exploring objects that are inherently infinite. 

A famous motivation for Dedekind was to prove that $\sqrt{2} \sqrt{3} = \sqrt{6}$. How might that go with the relations? The Yes intervals for $\sqrt{p}$ are those intervals $\{0, a\} \lte b$ that satisfy $\max(0,a)^2 \rint  p \rint  b^2$. Let $0\rint 0 < a \lte b$ be a $\sqrt{2}$-Yes interval and $0\rint 0 < c\lte d$ be a $\sqrt{3}$-Yes interval. Multiplication of them leads to $ac \lte bd$. Squaring the multiplication rational betweenness relation leads to intervals, where primes are used to denote another set of intervals satisfying the same conditions as their unprimed counterparts,  of the form $aa' cc' \lte bb' dd'$. Since $aa' \lte 2 \lte bb'$ and $cc' \lte 3 \lte dd'$, multiplication leads to $aa' cc' \lte 6 \lte bb' dd'$. Thus, $\sqrt{2} \sqrt{3} = \sqrt{6}$. This can obviously be generalized to any $n$-th roots. 

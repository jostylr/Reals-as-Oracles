...
The idea for the returned intervals is that they are considered to contain the real number. Any interval $c\rint d$ that appears as the second entry of a core output is a \textbf{prophecy} of the oracle and it will be denoted as $c\rint d \in R$. 

Writing $R(a\rint b, \delta)=(k ,A)$ implies that one of the possible core outputs for that core input is $(k, A)$. The expression $R(a\rint b, \delta) = c\rint d$ will be shorthand to imply that one of the outputs for that pair of inputs is $(1, c\rint d$).  The expression $R(a \rint b, \delta) = k$ implies that there is an output pair for that input such that $k$ is the first entry of the pair. The expression $R(a\rint b, \delta) \neq k$ means that none of the outputs for that input pair has $k$ as its first entry.  

The notation $\mathbb{I}_R$ represents the set of all intervals that contain a prophecy. 

To be an oracle, the procedure must satisfy six properties.  The properties are
\begin{enumerate}
    \item Range. If $R(a\rint b, \delta) = (k, c\rint d)$, then one of the following must hold
    \begin{itemize}
        \item $k=1$. $c\rint d$ is a subinterval of $\halo{a\rint b}$ and intersects $a\rint b$. 
        \item $k=0$. $c\rint d$ is not contained in $\halo{a\rint b}$ but does intersect $a \rint b$.
        \item $k=-1$. $c\rint d$ is disjoint from $a\rint b$.
    \end{itemize} 
    If $R(a \rint b, \delta) = (n, \emptyset)$, then $k = -1$. Furthermore, for $a:b$ and $\delta$, there exists at least one output pair such that either $k=1$ or $k=-1$.

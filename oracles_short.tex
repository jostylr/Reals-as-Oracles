...
Rational numbers satisfy the fact that, given three distinct rational numbers, $a, b, c$, exactly one of the following holds true\rint  $a\rint b\rint c$, $a\rint c\rint b$, or $b\rint a\rint c$. That is, exactly one of them is between the other two. It can be written in notation as $a\rint b\rint c$ holds true if and only if both \sout{$a\rint c\rint b$} and \sout{$b\rint a\rint c$} hold true. This follows from the pairwise ordering of each of them as provided by the Trichotomy property for rational numbers along with the Transitive property. 
...
\begin{enumerate}
   ...
   If $a\rint b \in R$ and $c\rint d$ is disjoint from $a\rint b$, then there exists a $\delta$ such that $R(c\rint d, \delta) = \emptyset$.
    ...
    \item Closed. 
    If $\halo{a} \in \mathbb{I}_R$ for all $\delta $, then $R(a\rint b, \delta) \neq \emptyset$ for all $\delta$ and $b$. Such an $a$ is called a \textbf{root} of the oracle. 
\end{enumerate}
...
The range of $R$ can be thought of as intervals that are known to include the real number through some computation or property. The set $\mathbb{I}_R$ contains all the intervals that contain an interval in the range of $R$. Consistency ensures that the empty set is not assigned to any such interval and this is possible since all intervals in $\mathbb{I}_R$ contain an interval that can be returned for any $\delta$.
...
A rational interval $a\rint b$ is a \textbf{Yes interval} of the oracle $R$ if $R(a\rint b, \delta) \neq \emptyset$ for all $\delta >0$. This includes all intervals that are in  $\mathbb{I}_R$, but it also includes the \textbf{$a$-rooted} intervals where $a$ is a root of the oracle.   A rational interval $a\rint b$ is a \textbf{No interval} if $a\rint b$ is disjoint from a prophecy. The oracle property Disjointness then implies $R(a\rint b, \delta) = \emptyset$ for some $\delta > 0$. In the examples below, the way the empty set arises is that a prophecy is generated by the procedure which is disjoint from the interval in question. It is not the case that an oracle applied to a No interval always returns an empty set; it just has to at least once. 

In a nonconstructive sense, each interval $a\rint b$ is either a Yes interval or a No interval. If an interval is disjoint from a prophecy, it is a No interval and if it contains a prophecy, then it is a Yes interval. But it is possible for an interval to intersect all prophecies without containing any of them. Thus, testing a given interval against a finite amount of prophecies may be indeterminate. It is the case that one can argue nonconstructively that if an interval intersects every prophecy, it is a Yes interval; see Proposition \ref{os:inter}. Both knowing that it intersects every prophecy and the proposition itself are nonconstructive ideas. 

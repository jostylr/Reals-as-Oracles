...
Higher mathematics is not much help here. The common suggestion to dedicated students of mathematics is to define real numbers using Cauchy sequences or Dedekind cuts. It would be hard to imagine this being a successful explanation to seventh graders, particularly before they have mastered something like square roots. Infinite decimals are not easy either, particularly for those who want to write it all down. The idea of arithmetic with something which goes on forever to the right would seem to be both nonsense and confusing to actually do. 

I think there is a better way of handling this, one which is approachable to students learning about real numbers for the first time and which can be delved into later in all its glory for more advanced students. The approach is that of oracles which could also be viewed as an infinite version of the 20 questions game\footnote{\url{https://en.wikipedia.org/wiki/Twenty_questions}}. 

\section{Introducing the Oracles}

If we talk about the square root of 2 being 1.41 or so, what we probably either mean that it is between 1.41 and 1.42 or between 1.405 and 1.415. We think we should foster the idea of thinking in terms of intervals delimited by rational numbers. 

My proposal is that a real number, such as square root 2, is an object which should be thought of as being in a bunch of rational intervals. Its core job is to say Yes or No when presented with an interval. It says Yes if it is ``in'' the interval, and No if it is not. I call it an oracle. 
